...
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
mkdir -p "$TMP/Kodosi/scripts" "$TMP/kodosiSwift/scripts"
...
cp "$SWIFT_ROOT/scripts/verify-ghostty-lock-parity.sh" "$TMP/kodosiSwift/scripts/"
...
KODOSI_DIR="$TMP/Kodosi" bash "$TMP/kodosiSwift/scripts/verify-ghostty-lock-parity.sh"
...
expect_failure \
    ...
    bash "$TMP/kodosiSwift/scripts/verify-ghostty-lock-parity.sh"
