    ...
    ILogger<ConnectionLease> logger) : IHostedService, IAsyncDisposable
{
    ...
    public async Task StartAsync(CancellationToken ct)
    {
        ...
        if (await command.ExecuteScalarAsync(ct) is not true)
        {
            ...
            throw new InvalidOperationException("Another Kodosi backend owns the terminal connection lease.");
        }
    ...
    private async Task MonitorAsync(NpgsqlConnection active, CancellationToken ct)
    {
        ...
        {
            connections.StopAll();
            logger.LogCritical(error, "Terminal connection lease was lost; stopping the backend."); lifetime.StopApplication();
        }
    }
