...
if [ -z "$target" ]; then
  case "$(uname -s)-$(uname -m)" in
    ...
    *)
      printf 'unsupported binding host: %s-%s\n' "$(uname -s)" "$(uname -m)" >&2
      exit 1
      ;;
  esac
fi
...
case "$target" in
  macos-arm64)
    ...
    output="$crate_dir/src/bindings_macos.rs"
    ;;
  linux-x86_64)
    layout=linux-x86_64
    clang_target=
    output="$crate_dir/src/bindings_linux.rs"
    ;;
  *)
    printf 'unsupported binding target: %s\n' "$target" >&2
    exit 1
    ;;
esac
header="$KODOSI_GHOSTTY_DIR/Vendor/GhosttyVt/$layout/include/ghostty/vt.h"
include_dir="$KODOSI_GHOSTTY_DIR/Vendor/GhosttyVt/$layout/include"
