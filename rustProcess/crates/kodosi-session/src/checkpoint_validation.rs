...
use kodosi_domain::terminal::{
    TERMINAL_SEMANTIC_CHECKPOINT_MAX_BYTES, TerminalCheckpointV2, TerminalScreen,
};
use kodosi_utils::{KodosiError, Result};

use crate::terminal::TerminalHistoryPolicy;
...
mod tests {
    ...
    fn checkpoint_fixture(terminal: &mut Terminal) -> TestResult<TerminalCheckpointV2> {
        let semantic = terminal.semantic_checkpoint(CheckpointLimits {
            max_json_bytes: TERMINAL_SEMANTIC_CHECKPOINT_MAX_BYTES,
            ..CheckpointLimits::default()
        })?;
        let state = terminal.state()?;
        Ok(TerminalCheckpointV2::new(
            kodosi_domain::terminal::TerminalSize::new(state.rows, state.cols)?,
    ...
    fn validator_accepts_valid_and_rejects_malformed_or_contradictory_checkpoints() -> TestResult {
        let malformed = TerminalCheckpointV2::new(
            kodosi_domain::terminal::TerminalSize::new(4, 12)?,
        ...
        let contradictory = TerminalCheckpointV2::new(
            kodosi_domain::terminal::TerminalSize::new(5, 12)?,
