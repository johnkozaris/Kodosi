use super::{
    CancellationToken, Credentials, Duration, Error, Network, Result, Value, invalid, json, relay,
    wire,
};
...
impl Network {
    ...
    async fn notification_connection(
        ...
        let (mut socket, _) =
            relay::socket(self, &credentials, "ws/events", "events", None, None).await?;
        for surface in ["sessions", "friends", "devices", "rooms"] {
        ...
        loop {
            tokio::select! {
                ...
                _=heartbeat.tick()=>relay::send_json(&mut socket,json!({"type":"ping"})).await?,
    ...
    async fn refresh_surface(&self, admitted: &Credentials, surface: &str) -> Result<()> {
        ...
        let events = match surface {
            ...
            "rooms" => vec![self.room_list().await?],
