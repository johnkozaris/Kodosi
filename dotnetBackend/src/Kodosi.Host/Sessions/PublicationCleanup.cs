...
public sealed class PublicationCleanup(IServiceScopeFactory scopes, AdmissionGate gate, RelayDirectory relay,
    TimeProvider clock, ILogger<PublicationCleanup> logger) : BackgroundService
{
    ...
    internal async Task SweepAsync(CancellationToken ct)
    {
        ...
        var online = relay.OnlineSessionIds();
        ...
        foreach (var session in expired) relay.RemoveSession(session.Id);
        await store.Sessions.Where(s => ids.Contains(s.Id)).ExecuteDeleteAsync(ct);
        foreach (var user in users.Union(expired.Select(s => s.OwnerUserId))) relay.Notify(user, "sessions");
    }
}
