...
public sealed class RelayDirectory(TimeProvider clock)
{
    ...
    internal sealed class MutationRecovery(RelayDirectory directory) : IDisposable
    {
    ...
    {
        if (sessions.Count >= 4096 && !sessions.ContainsKey(state.Id)) throw new ApiException(503, "Relay session capacity reached.");
        LiveSession live;
        while (true)
        {
