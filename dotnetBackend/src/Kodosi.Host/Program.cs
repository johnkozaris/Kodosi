...
builder.Services.AddSingleton<RelayDirectory>();
builder.Services.AddHostedService<RelayLease>();
builder.Services.AddHostedService<RelayMaintenance>();
...
app.MapGet("/health/live", () => Results.Ok(new { status = "ok", apiContractVersion = 15, authContractVersion = 1 }));
app.MapGet("/health/ready", async (KodosiDbContext db, CancellationToken ct) =>
    await db.Database.CanConnectAsync(ct) ? Results.Ok(new { status = "ok", apiContractVersion = 15, authContractVersion = 1 }) : Results.StatusCode(503));
...
app.MapRelay();
app.Lifetime.ApplicationStopping.Register(() => app.Services.GetRequiredService<RelayDirectory>().StopAll());
app.Run();
