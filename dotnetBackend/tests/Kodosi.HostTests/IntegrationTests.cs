...
public sealed class IntegrationTests(PostgresFixture postgres)
{
    ...
    public async Task StalePreparedProofCannotMutateAfterDeviceRevocation()
    {
        ...
        Assert.Empty(await scope.ServiceProvider.GetRequiredService<KodosiDbContext>().Rooms.ToListAsync(TestContext.Current.CancellationToken));
    }
    ...
    public async Task RealSocketJourneySharesOnlyChosenSessionAndRevocationEndsControl()
    {
        ...
        await host.SendAsync(new ArraySegment<byte>(RelayTests.Frame(3, generation, 0, 1, 0)), WebSocketMessageType.Binary, true, TestContext.Current.CancellationToken);
        ...
        var checkpoint = RelayTests.Frame(3, generation, 1, 2, 0);
        ...
        var raw = RelayTests.Frame(4, generation, 0, 0, 1);
