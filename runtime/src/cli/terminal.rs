...
pub(super) async fn attach(root: &Path, session: Uuid) -> Result<()> {
    ...
    let mut client = local_host::TerminalClient::connect(root, session).await?;
    let initial = tokio::time::timeout(
        Duration::from_secs(5),
        local_host::read_terminal(&mut client.reader),
    )
    .await
    ...
    loop {
        tokio::select! {
            ...
            _=resize.recv()=>{if let Ok((cols,rows))=crossterm::terminal::size(){local_host::write_resize(&mut client.writer,cols,rows).await?;}},
